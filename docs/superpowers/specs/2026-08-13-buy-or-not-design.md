...
3. 미스면 정적 HTML 요청 후 cheerio 파싱. `__NEXT_DATA__` 스크립트 블록(`dehydratedState.queries`) 안에 상품명·브랜드·가격·이미지·카테고리·옵션이 JSON으로 들어있어 이 필드들은 이 단계에서 대부분 성공한다 (Phase 0 조사 결과, `docs/superpowers/notes/phase0-musinsa-findings.md` 참고).
4. **`sizeTable`(실측 표)만은 별도 경로다.** Phase 0에서 실측 표를 만드는 내부 API 엔드포인트를 찾지 못했다 — 브라우저 네트워크 요청·Resource Timing을 직접 감시해도 잡히지 않았고, 정적 HTML에도 없다. 그래서 `sizeTable` 필드는 처음부터 자동 파싱을 시도하지 않고 곧바로 §8-1 붙여넣기 폴백으로 보낸다.
5. 요청에는 timeout과 재시도 1회를 둔다. 파싱 결과는 성공·부분성공 모두 캐시에 저장한다.
...
**이 계산에서 `sizeTable`과 `options`는 제외한다.** 이 둘은 처음부터 자동 파싱을 시도하지 않으므로(§8-1, §8) 항상 "실패"인 게 정상이다 — 포함시키면 모든 옷이 영원히 `manual`로 찍혀 지표가 무의미해진다. `parse_mode`는 상품명·브랜드·가격·이미지·카테고리 5개 필드만 보고, 이 5개가 갑자기 실패하기 시작할 때만 "무신사가 개편됐다"는 신호로 읽는다.

### 8-1. 사이즈표: 붙여넣기 자동 파싱

`sizeTable`은 자동 파싱을 시도하지 않으므로(위 §8 참고), 실패 폼 대신 **붙여넣기 텍스트 영역**을 먼저 보여준다: "무신사 '사이즈' 탭에서 표 전체(헤더 행부터 사이즈 행까지)를 드래그해 복사해서 붙여넣어주세요."

**파싱**: 붙여넣기 이벤트에서 `clipboardData.getData('text/html')`를 먼저 확인한다 — 브라우저가 표를 복사하면 `<table><tr><td>` 구조가 그대로 담기므로 셀 경계가 정확하다. 비어있으면(모바일 등) `text/plain`을 탭/공백 기준으로 나누는 방식으로 폴백한다. 헤더 셀 텍스트는 `lib/musinsa/measurements.ts`의 별칭 테이블로 표준 항목명에 매칭한다 — 스크래퍼가 HTML에서 라벨을 정규화할 때 쓰는 것과 **같은 테이블**이라 새 정규화 로직이 필요 없다. 매칭 안 되는 헤더는 버리지 않고 원문 키로 저장한다(기존 원칙, §7 참고).

**실패 시**: 사이즈 행/헤더를 하나도 못 찾으면 안내 문구와 함께 기존 필드별 수동 입력 폼(§8 폴백)을 그대로 노출한다. 헤더는 인식했는데 사용자가 고른 사이즈 행이 없으면, 인식된 다른 사이즈는 보여주되 그 사이즈 값만 수동 입력을 요청한다.

**캐시 반영**: 파싱 성공 시 사이즈×항목 매트릭스 전체를 `musinsa_cache`에 반영한다 — 다음에 같은 상품 링크를 넣는 사용자는 이 단계 자체를 건너뛴다. 사용자가 고른 사이즈의 행만 `garment_measurements`에 쓴다. 이렇게 채워진 결과는 `parse_mode = 'manual'`로 기록해 자동 파싱(`auto`)과 구분한다 — 사용자가 붙여넣은 값이라는 출처를 잃지 않기 위함이며, §8의 `manual` 비율 관측 지표도 그대로 유지된다.
...
사용자가 붙여넣은 개별 상품 페이지의 단건 조회만 수행한다. 목록 크롤링이나 대량 수집은 하지 않는다. 캐시를 우선 조회하고, robots.txt를 확인해 금지 경로는 요청하지 않는다.

무신사 robots.txt는 이름이 지정된 봇(주요 검색엔진·`Claude-User` 등)만 `Allow`하고, 나머지 모든 User-Agent는 `Disallow: /`로 막는 와일드카드 규칙을 두고 있다(Phase 0 조사, `docs/superpowers/notes/phase0-musinsa-findings.md`). 이 앱의 서버 fetcher는 지정 목록에 없으므로 robots.txt 기준으로는 "허용 안 됨" 범주에 들어간다. robots.txt는 법적 구속력이 아니라 관례이고, 사용자가 자신이 볼 상품 링크 하나를 개인 용도로 조회하는 것은 대량 크롤링과 성격이 다르지만, 비상업적 개인 프로젝트라도 이 리스크가 0은 아니라는 점을 인지한 상태로 진행한다.
...
성공 집합의 항목별 값을 **여러 개의 선호 범위(클러스터)**로 묶는다. 크롭과 오버핏처럼 서로 다른 극단을 동시에 선호하는 사용자를 하나의 최소·최대 범위로 잡으면, 그 사이의 좋아하지 않는 중간 기장까지 통과 판정을 받아버리기 때문이다 — 단순 평균은 물론 단일 범위로도 이 경우를 표현할 수 없다.

클러스터링 방법:

1. 성공 집합의 값을 오름차순 정렬한다.
2. 인접한 두 값의 차이가 그 항목의 허용 편차(`t`, `lib/fit/rules.ts`)보다 크면 그 지점에서 구간을 나눈다.
3. 나뉜 각 구간의 최소·최대가 하나의 **선호 범위 `[lo_k, hi_k]`** 다 — 항목당 범위가 1개일 수도, 여러 개일 수도 있다.

예: 총장 성공 집합이 `{58, 60, 62, 78, 80}`이고 `t=3`이면 62→78 구간(간격 16)이 `t`보다 커서 `[58, 62]`(크롭)와 `[78, 80]`(오버핏) 두 범위로 나뉜다. 반대로 값이 촘촘히 모여 있으면 범위는 하나로 남는다 — 취향이 한 곳에 몰린 사용자에게 불필요한 복잡도를 강요하지 않는다.
...
- 3벌 이상이지만 성공 집합이 비어 있으면, 같은 카테고리 전체 `owned` 옷으로 선호 범위(들)를 대체하고 리포트에 신뢰도 `low`를 표시한다. 이 대체 집합에도 동일한 클러스터링을 적용한다.
...
후보 옷과 선호 범위에 **모두 존재하는 표준 항목**에 대해서만 계산한다. 한 항목에 선호 범위가 여러 개(클러스터)일 수 있으므로, **각 범위에 대해 편차를 계산하고 그중 가장 작은 값(가장 가까운 범위 기준)**을 그 항목의 최종 편차로 쓴다 — 크롭 범위와 오버핏 범위를 둘 다 가진 사용자에게 후보 옷이 둘 중 하나에만 맞아도 통과해야 하기 때문이다.

```
범위가 k개(lo_1..hi_1, lo_2..hi_2, ...)일 때, 범위마다:

  허용 구간_i = [lo_i - t, hi_i + t]    (t = 허용 편차)

  v ∈ 허용 구간_i          → excess_i = 0
  v < lo_i - t             → excess_i = (lo_i - t) - v
  v > hi_i + t             → excess_i = v - (hi_i + t)

excess = min(excess_1, excess_2, ...)   ← 가장 잘 맞는 범위 기준
...
범위가 하나뿐인 항목(대부분의 경우)은 기존과 동일하게 동작한다 — 이 공식은 단일 범위의 일반화다.
...
| 실측 표(`sizeTable`) 파싱 실패 | 붙여넣기 자동 파싱(§8-1) 시도 → 그마저 실패하면 필드별 수동 입력 폼 |
