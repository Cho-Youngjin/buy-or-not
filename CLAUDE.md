...
- **프론트엔드 디자인은 `design-taste-frontend-v1` 스킬을 사용한다.** `.agents/skills/design-taste-frontend-v1`에 설치되어 있고 `.claude/skills/`에 심볼릭 링크로 연결되어 있다 (출처: `https://github.com/Leonxlnx/taste-skill`, `npx skills add`로 설치, `skills-lock.json`에 버전 고정). 화면·컴포넌트를 만들거나 스타일을 정할 때 이 스킬을 로드해서 따른다. 같은 저장소에서 함께 설치된 다른 12개 스킬(`design-taste-frontend`(v2), `minimalist-ui`, `industrial-brutalist-ui` 등)은 이 프로젝트의 기본값이 아니다 — 사용자가 명시적으로 지정하지 않는 한 v1을 쓴다.
...
- **커밋·PR에 AI 기여자 표시를 넣지 않는다.** `Co-Authored-By: Cursor`, `Co-Authored-By: Claude`, `Made with Cursor` 같은 트레일러는 GitHub Contributors에 에이전트가 올라가게 한다. 커밋 메시지는 사용자 이름으로만 남긴다.
